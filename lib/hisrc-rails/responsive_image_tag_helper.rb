...
module HisrcRails
  module ResponsiveImageTagHelper
    ...
    #    <img src="rails.png" data-1x="rails.png" data-2x="rails@2x.png" />
    ...
    def responsive_image_tag(src, options = {})
      options[:data] ||= {}
      options[:data][:'1x'] ||= options.delete(:'1x').presence || src
      options[:data][:'2x'] ||= options.delete(:'2x').presence || src.gsub(/([\w\/]+).(\w+)/, '\1@2x.\2')
